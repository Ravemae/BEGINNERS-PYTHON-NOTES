...
def id_generator():
    ...
    random_integers = ''.join(str(r.randint(0, 9)) for _ in range(6))
...
def update():
    ...
    if not selected_item:
        ...
        database2.update_employee(name, role, gender, status, contact, email, id)
        add_to_treeview()
        clear()
...
def insert():
    ...
    if not (id and name and role and gender and status and contact and email):
        ...
        database2.insert_employee(id, name, role, gender, status, contact, email)
        add_to_treeview()
